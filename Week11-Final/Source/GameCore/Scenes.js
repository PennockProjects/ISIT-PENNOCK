...
Crafty.scene('Victory', function() {'use strict';
	...
	Crafty.e('2D, DOM, Text').attr({
		x : 0,
		y : 0
	}).text('You are victorious!').textColor('#FFFFFF').textFont({
		size : '48px',
		weight : 'bold'
	});
...
// Draw the initial game state
Crafty.scene('SecondLevel', function() {'use strict';
...
Crafty.scene('Failure', function() {'use strict';
	...
	Crafty.e('2D, DOM, Text').attr({
		x : 0,
		y : 0
	}).text('You are Dead!').textColor('#FF0000').textFont({
		size : '48px',
		weight : 'bold'
	});
